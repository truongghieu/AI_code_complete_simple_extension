...
import { getChatPanelTemplate } from './chatPanelTemplate';
...
export class ChatPanel {
  ...
  private readonly _apiClient: ClaudeApiClient;
  ...
  public constructor(panel: vscode.WebviewPanel, apiClient: ClaudeApiClient) {
    this._panel = panel;
...
Please provide instructions on how you'd like to improve or modify this code.`
  ...
  public static createOrShow(extensionUri: vscode.Uri) {
    ...
    ChatPanel.currentPanel = new ChatPanel(panel, new ClaudeApiClient(apiUrl, model));
...
Use these commands to directly modify code or create files as needed.`;
   ...
   */
  private _parseAndApplyEditCommands(response: string): boolean {
   ...
   */
  private _executeEditCommand(commandContent: string): string {
    ...
    if (deleteLinesMatch) {
      return this._executeDeleteLinesCommand(
        parseInt(deleteLinesMatch[1], 10),
        parseInt(deleteLinesMatch[2], 10)
      );
    }
   ...
   */
  private _executeReplaceCommand(oldText: string, newText: string): string {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
   ...
   * @returns A string describing the result
   */
  private _executeInsertAtLineCommand(text: string, lineNumber: number): string {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      throw new Error('No active editor found');
    }
   ...
   * @returns A string describing the result
   */
  private _executeInsertAtCursorCommand(text: string): string {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      throw new Error('No active editor found');
    }
   ...
   * @returns A string describing the result
   */
  private _executeDeleteLinesCommand(startLine: number, endLine: number): string {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      throw new Error('No active editor found');
    }
   ...
   * @returns A string describing the result
   */
  private _executeCreateFileCommand(filePath: string, content: string): string {
   ...
   * @returns A string describing the result
   */
  private _executeOpenFileCommand(filePath: string): string {
    // Normalize the path and make it absolute if it's relative
    let normalizedPath = filePath;
    if (!path.isAbsolute(normalizedPath)) {
      // If the path is relative, make it relative to the workspace root
   ...
   */
  private async _handleLazyEditMessage(text: string): Promise<void> {
    // Add user message to chat
    this._messages.push({ role: 'user', content: text });
    this._updateWebview();
...
\`\`\`${this._lazyEditState.language}`;
   ...
   */
  private _processLazyEditResponse(editedCode: string): string {
...
// Updated _getWebviewContent method with improved code formatting
    ...
    return getChatPanelTemplate(modelSelectorHtml, messageHtml);
  }
 ...
 */
private _convertTextListsToHtml(text: string): string {
