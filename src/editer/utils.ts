...
import { UNCHANGED_CODE } from './constants';
 ...
 */
export async function applyEditToEditor(
 ...
 */
export function createLazyEditPrompt(
  originalCode: string, 
  filename: string, 
...
When parts of the code remain unchanged, you may indicate this with a comment that says "${UNCHANGED_CODE}" instead of rewriting that section.
Keep at least one line above and below from the original code when using "${UNCHANGED_CODE}", so that we can identify what the previous code was.
Do not place "${UNCHANGED_CODE}" comments at the top or bottom of the file when there is nothing to replace them.
 ...
 */
export function processLazyEditResponse(originalCode: string, editedCode: string): string {
